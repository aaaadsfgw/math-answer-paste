...
- `Ctrl+Shift+C` で、選択中の問題文を解いて最終回答だけをクリップボードへ入れられます。
...
1. Webページ上の問題文を選択します。
2. `Ctrl+Shift+C` を押します。
3. 拡張機能が選択テキストを取得し、Ollama に送信します。
4. 最終回答だけをクリップボードに入れます。
5. ユーザーは `Ctrl+V` で任意の場所に貼り付けます。

通常のコピー操作に完全に割り込むのではなく、拡張機能専用のショートカットとして動作します。ショートカットが他の機能と衝突する場合は、`chrome://extensions/shortcuts` から変更してください。
...
- `Ctrl+Shift+C` が反応しない: `chrome://extensions/shortcuts` でショートカット割り当てを確認してください。
